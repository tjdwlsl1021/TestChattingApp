...
import com.example.testchattingapp.databinding.ActivityRegistrationBinding
    ...
    private val getContent =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
            if(result.resultCode == RESULT_OK) {
                ...
                binding.registrationIv.setImageURI(imageUri) //이미지 뷰를 바꿈
                Log.d("이미지", "성공")
            }
            else{
                Log.d("이미지", "실패")
                            ...
                            if (task.isSuccessful) {
                                ...
                                FirebaseStorage.getInstance()
                                        .reference.child("userImages").child("$userIdSt/photo").putFile(imageUri!!).addOnSuccessListener {
                                            ...
                                            errorMsg -> Log.e(TAG, "${errorMsg.printStackTrace()}")
                                    }
